...
- **Deep Research Mode**: Available for Research/Enterprise tier users. Uses Google's Gemini Deep Research Agent API (agent: `deep-research-pro-preview-12-2025`, Interactions API at `v1beta/interactions`). Async polling with 5s interval. Toggle button in chat header turns violet when active. Shows animated "Researching..." progress bubble. Messages stored in conversation on completion. Same GEMINI_API_KEY used.
