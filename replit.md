...
- **Paid-tier gating restored (May 30, 2026)**: The temporary "LAUNCH NIGHT" relaxations that let free-tier visitors use paid features (Deep Think / verification engine and the research/pro models) have been fully reverted, so paid tiers again require payment via PayPal. Reverted: `DEEP_QUOTA` back to `free: 0, pro: 0` in `server/services/reasoning-engine.ts`; legacy `/messages` verify gate restored to `if (senderTier === 'free') return "unknown";`; streaming `/messages/stream` `tierBlocksDeep` back to `effectiveTier === 'free' || effectiveTier === 'pro'`; `isResearchOrAbove` and `handleModelChange` upgrade popups restored in `client/src/pages/chat.tsx`. Additionally hardened the legacy `/messages` route to clamp the client-supplied `selectedModel`/`deepThink` to the user's tier server-side (was client-gated only) so a free user can't bypass the UI and POST a paid model directly. Promo codes, enterprise team codes, Stripe, and the PayPal checkout/sync flow are unchanged — only the free-access launch flags were removed.
