...
- **Code Studio** (`/code-studio`): Full in-browser IDE — **$15/month add-on**, available to any subscription tier. Includes **15 AI credits/month** (resets each billing cycle). Each AI build uses 1 credit. When credits run out, users are prompted to buy more. Credit packs: 15/$15, 25/$25, 45/$45, 100/$100, 250/$250, 500/$500 — one-time PayPal Orders v2 payments, credits never expire. Supports promo code entry on the paywall — 100% off codes activate access for free (no PayPal). Backend route: `POST /api/apply-code-studio-promo`. Owner lifetime code seeded on startup: `TURBOCODE-FOREVER`. Monaco editor (VS Code engine) with multi-file support, file tabs, syntax highlighting for 8+ languages. Live preview served from real URL `/code-preview/:id` (no sandbox restrictions) after build; srcDoc used only for live editing. Code execution via Piston API. Gemini 3.1 Pro AI sidebar. Save projects to PostgreSQL DB. Deploy to a public URL at `/p/:slug`. Backend gate: all `/api/code/*` routes require `codeStudioAddon === true`. Monthly credit reset: 30 days after last reset, 15 credits restored automatically. Credit routes: `GET /api/code/credits`, `POST /api/code/buy-credits`, `POST /api/code/capture-credits`. Schema: `codeStudioAddon`, `codeStudioAddonSubId`, `codeStudioCredits` (integer), `codeStudioCreditsResetAt` (timestamp).
