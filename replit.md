...
- **External Construction API**: RESTful API system for integrating TurboAnswer's AI into external construction/repair websites. Features: API key management (generate/list/delete keys), SHA-256 hashed key storage, tiered daily rate limiting, monthly $25 budget metering with auto-reset, admin vs public key types, usage logging. Two key types: **Public** (consumer-friendly data for websites) and **Admin** (employee-only with supplier pricing, profit margins, internal cost breakdowns — requires admin role). Five endpoints authenticated via `Authorization: Bearer ta_...` or `X-Api-Key` header:
  - `POST /api/v1/construction/analyze` — AI image analysis with 6 types (damage_assessment, material_identification, repair_estimate, safety_inspection, progress_tracking, general). Gemini 2.5 Flash vision. 5¢/call.
  - `POST /api/v1/construction/advice` — Expert advice across 14 categories. 2¢/call.
  - `POST /api/v1/construction/schedule` — AI work scheduling with dependencies, critical path, crew recs. Optional site photo. 8¢/call.
