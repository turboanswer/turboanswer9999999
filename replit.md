...
- **Code Studio** (`/code-studio`): Full in-browser IDE — **$15/month add-on** with **7-day free trial** (5 trial credits, $0 for 7 days, then $15/month). Available to any subscription tier. Includes **15 AI credits/month** (resets each billing cycle — credits do NOT roll over). Each AI build uses 1 credit. When credits run out, users are prompted to buy more. Credit packs: 15/$15, 25/$25, 45/$45, 100/$100, 250/$250, 500/$500 — one-time PayPal Orders v2 payments, pack credits never expire. Supports promo code entry on the paywall — 100% off codes activate access for free (no PayPal). Backend route: `POST /api/apply-code-studio-promo`. Owner lifetime code seeded on startup: `TURBOCODE-FOREVER`. Monaco editor (VS Code engine) with multi-file support, file tabs, syntax highlighting for 8+ languages. Live preview served from real URL `/code-preview/:id` (no sandbox restrictions) after build; srcDoc used only for live editing. Code execution via Piston API. Gemini 3.1 Pro AI sidebar. Save projects to PostgreSQL DB. Deploy to a public URL at `/p/:slug`. Backend gate: all `/api/code/*` routes require `codeStudioAddon === true`. Credit reset logic: `codeStudioCreditsResetAt` stores the NEXT reset date; when that date passes, credits reset to exactly 15 (no rollover) and next reset = now + 30 days. On trial signup: 5 credits, nextReset = now + 7 days. Credit routes: `GET /api/code/credits`, `POST /api/code/buy-credits`, `POST /api/code/capture-credits`. Schema: `codeStudioAddon`, `codeStudioAddonSubId`, `codeStudioCredits` (integer), `codeStudioCreditsResetAt` (timestamp = next reset date). Header shows a credit percentage bar (% of 15 used) instead of a badge.
