...
export default function WorkgroupsPage() {
  ...
  const { data: chatMessages = [], refetch: refetchMessages } = useQuery<any[]>({
    ...
    enabled: !!activeWgId && (activeTab === 'chat' || activeTab === 'shared'),
  ...
  const generateCodeMutation = useMutation({
    mutationFn: async () => {
      const res = await apiRequest('POST', `/api/workgroups/${activeWgId}/generate-code`);
      return res.json();
    },
    onSuccess: (data: any) => {
          ...
          {isLoading ? (
            ...
            <div className="space-y-1">
              {workgroups.map((wg: any) => (
                <button key={wg.id} onClick={() => { setActiveWgId(wg.id); setActiveTab('chat'); setGeneratedCode(null); }}
      ...
      <div className="flex-1 overflow-hidden flex flex-col">
        ...
        {activeTab === 'shared' && (
          <div className="flex-1 overflow-y-auto px-3 py-3">
            {(() => {
              const sharedMessages = chatMessages.filter((m: any) => m.content?.includes('Shared from TurboAnswer'));
              if (sharedMessages.length === 0) {
                return (
                  <div className="text-center py-16">
                    <div className={`w-16 h-16 rounded-full mx-auto mb-4 flex items-center justify-center ${isDark ? 'bg-white/5' : 'bg-gray-100'}`}>
                      <Share2 className="h-7 w-7 opacity-40" />
                    </div>
                    <p className={`text-sm font-medium ${textSecondary}`}>No shared responses yet</p>
                    <p className={`text-xs mt-1 ${isDark ? 'text-[#555555]' : 'text-gray-400'}`}>
                      Share AI responses from your chat using the share button
                    </p>
                  </div>
                );
              }
              return (
                <div className="space-y-3">
                  <p className={`text-xs font-medium px-1 ${textSecondary}`}>{sharedMessages.length} shared response{sharedMessages.length !== 1 ? 's' : ''}</p>
                  {sharedMessages.map((m: any) => {
                    const parts = m.content.split('\n\n');
                    const questionPart = parts.find((p: string) => p.startsWith('❓'))?.replace('❓ Question:\n', '') || '';
                    const answerPart = parts.find((p: string) => p.startsWith('💡'))?.replace('💡 AI Response:\n', '') || '';
                    return (
                      <div key={m.id} className={`rounded-2xl border p-4 ${isDark ? 'bg-[#111111] border-[#222222]' : 'bg-white border-gray-200'}`}>
                        <div className="flex items-center gap-2 mb-3">
                          <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold text-white ${getAvatarColor(m.senderName || '?')}`}>
                            {(m.senderName || '?')[0].toUpperCase()}
                          </div>
                          <div>
                            <p className="text-xs font-medium">{m.senderName}</p>
                            <p className={`text-[10px] ${textSecondary}`}>{new Date(m.createdAt).toLocaleString()}</p>
                          </div>
                        </div>
                        {questionPart && (
                          <div className={`rounded-xl p-3 mb-2 ${isDark ? 'bg-black border border-[#222222]' : 'bg-gray-50 border border-gray-200'}`}>
                            <p className={`text-[10px] font-semibold mb-1 ${isDark ? 'text-[#8ab4f8]' : 'text-blue-600'}`}>QUESTION</p>
                            <p className="text-sm whitespace-pre-wrap">{questionPart}</p>
                          </div>
                        )}
                        {answerPart && (
                          <div className={`rounded-xl p-3 ${isDark ? 'bg-[#0a1a0a] border border-[#1a331a]' : 'bg-green-50 border border-green-200'}`}>
                            <p className={`text-[10px] font-semibold mb-1 ${isDark ? 'text-emerald-400' : 'text-green-600'}`}>AI RESPONSE</p>
                            <p className="text-sm whitespace-pre-wrap leading-relaxed">{answerPart.slice(0, 500)}{answerPart.length > 500 ? '...' : ''}</p>
                          </div>
                        )}
                      </div>
                    );
          ...
          <div className="flex-1 overflow-y-auto">
            {isAdminUser && (
              <div className="px-4 pt-4 pb-2">
                <div className={`rounded-2xl border p-5 ${isDark ? 'bg-[#111111] border-[#222222]' : card}`}>
                  <div className="flex items-center justify-between mb-3">
                    <p className={`text-xs font-semibold uppercase tracking-wider ${textSecondary}`}>Invite Code</p>
                    <p className={`text-[10px] ${textSecondary}`}>{members.length}/20 members</p>
                  </div>
                  {generatedCode ? (
                    <div className="text-center">
                      <p className={`text-xs mb-3 ${textSecondary}`}>Share this code with people you want to invite</p>
                      <div className={`flex justify-center gap-2 mb-4`}>
                        {generatedCode.split('').map((d, i) => (
                          <div key={i} className={`w-11 h-13 flex items-center justify-center rounded-xl text-2xl font-bold ${isDark ? 'bg-black border border-[#333333] text-white' : 'bg-gray-100 border border-gray-300 text-black'}`}>
                            {d}
                          </div>
                        ))}
                      </div>
                      <div className="flex gap-2 justify-center">
                        <button onClick={() => { navigator.clipboard.writeText(generatedCode); toast({ title: "Code copied!" }); }}
                          className={`flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-medium transition-colors ${isDark ? 'bg-white/10 hover:bg-white/15 text-white' : 'bg-gray-100 hover:bg-gray-200 text-gray-900'}`}>
                          <Copy className="h-3.5 w-3.5" /> Copy Code
                        </button>
                        <button onClick={() => generateCodeMutation.mutate()}
                          className={`flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-medium transition-colors ${isDark ? 'bg-white/5 hover:bg-white/10 text-[#888888]' : 'bg-gray-50 hover:bg-gray-100 text-gray-600'}`}>
                    ...
                    <div className="text-center">
                      <p className={`text-xs mb-3 ${textSecondary}`}>Generate a 6-digit code to share with people</p>
                      <button onClick={() => generateCodeMutation.mutate()} disabled={generateCodeMutation.isPending || members.length >= 20}
                        className={`w-full py-3 rounded-xl text-sm font-semibold transition-all ${isDark ? 'bg-white text-black hover:bg-white/90' : 'bg-black text-white hover:bg-gray-900'} disabled:opacity-30`}>
                        {generateCodeMutation.isPending ? <Loader2 className="h-4 w-4 animate-spin mx-auto" /> : "Generate Invite Code"}
                      </button>
                      {members.length >= 20 && <p className="text-xs text-red-400 mt-2">Maximum 20 members reached</p>}
                    </div>
                  )}
                </div>
              </div>
